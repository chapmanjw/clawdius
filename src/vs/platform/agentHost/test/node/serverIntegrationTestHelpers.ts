 ...
 */
export async function startRealServer(options?: { readonly claudeSdkRoot?: string; readonly mockLlm?: boolean; readonly homeDir?: string; readonly userDataDir?: string; readonly env?: NodeJS.ProcessEnv; readonly capiReplay?: { readonly fixturePath: string; readonly mode?: CapiReplayMode; readonly workDir?: string; readonly real?: boolean }; readonly mockScenarios?: readonly IMockScenario[] }): Promise<IServerHandle> {
	...
	return new Promise((resolve, reject) => {
		...
		const childEnv = withAgentHostCoverage({
			...
			...(realCapture ? {
				...
				// that are "pro"-gated in the mock /models fixture, so mint a
				// pro-plan token for this harness.
				...
				// Route the agent host's shared CAPI client (used for model
				// discovery + requests) at the mock/proxy instead of
				// api.github.com, which would 401 with the fake token.
				VSCODE_AGENT_HOST_CAPI_URL_OVERRIDE: capiUrl,
			} : {}),
		});
