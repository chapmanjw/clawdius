...
// CLAWDIUS-BEGIN ultracode workflow transcript
...
export function formatWorkflowTranscript(jsonl: string, title?: string): string {
	...
	if (rendered === 0) {
		out.push(localize('ultracode.transcript.empty', "_No transcript has been recorded for this agent yet._"));
	}
	return out.join('\n');
}
...
export class WorkflowTranscriptContribution extends Disposable implements IWorkbenchContribution, ITextModelContentProvider {
	...
	static readonly ID = 'workbench.contrib.ultracode.workflowTranscript';
