...
suite('LocalAgentHostSessionsProvider', () => {
	...
	test('session type icons use per-agent codicons', () => {
		...
		const provider = createProvider(disposables, agentHost);
		// CLAWDIUS: `iconForAgentProvider` has no Codex/OpenAI branding case (the
		// fork removed Copilot/Codex) — an `openai` provider falls through to the
		// neutral base icon like any other unrecognized provider, same as
		// `unknown-agent`.
