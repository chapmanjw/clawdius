...
import { AgentHostByokModelsEnabledEnvVar, AgentHostClaudeAgentEnabledEnvVar, AgentHostIpcChannels, IAgentHostInspectInfo, IAgentHostSocketInfo, IAgentService, IConnectionTrackerService, isAgentEnabled } from '../common/agentService.js';
...
async function startAgentHost(): Promise<void> {
	...
	try {
		...
		// CLAWDIUS-END
		agentService = new AgentService(logService, fileService, sessionDataService, productService, gitService, rootConfigResource, telemetryService, fileMonitorService, fetchFn);
		...
		// Host-owned worktree isolation controller: a single instance drives folder / worktree isolation for
		// every agent, so providers stay unaware of it. Upstream builds its branch-name generator from the
		// Copilot API service; passing `undefined` selects the local AgentBranchNameGenerator, which derives a
		// branch name from the message text alone and makes no network call.
