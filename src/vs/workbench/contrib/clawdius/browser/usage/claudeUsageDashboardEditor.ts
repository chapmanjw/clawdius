...
// A native DOM editor pane (no webview => trivially zero-egress) that renders the Claude Code usage dashboard
// in the monospace terminal aesthetic of the CLI's /usage view: block-shade limit bars, a contribution
// heatmap, a braille Tokens-per-Day line chart, a model breakdown, and 24h activity. All data is the user's
// own local files (~/.claude/stats-cache.json + the on-demand capacity cache). `setInput` only READS local
// files (a restored editor never egresses at startup); the live /api/oauth/usage refresh is driven by the
// open action + the Refresh button. Cost is never shown - usage and limits only.
...
export class ClaudeUsageDashboardEditor extends EditorPane {
	...
	override async setInput(input: ClaudeUsageDashboardInput, options: IEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<void> {
		...
		await this.loadAndRender(token);
	}
	...
	private async loadAndRender(token: CancellationToken): Promise<void> {
	...
	private async refreshOnDemand(): Promise<void> {
		...
		try {
			...
			await this.loadAndRender(cts.token);
		} catch {
	...
	private renderHero(parent: HTMLElement, account: IClaudeAccount, refreshedAt: Date | undefined): void {
		...
		const privacy = refreshedAt
			? localize('clawdius.usage.dash.metaRefreshed', "Local history. Live limits last refreshed {0}.", refreshedAt.toLocaleString())
			: localize('clawdius.usage.dash.metaLocal', "Local history. Live limits refresh only when you open this dashboard.");
		append(text, h('.clawdius-usage-hero-meta')).textContent = privacy;
