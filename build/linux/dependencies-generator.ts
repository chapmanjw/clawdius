...
// CLAWDIUS: warn (do not fail) on dependency drift. The reference dep-lists.ts is
// upstream's; a fork's computed .deb/.rpm deps legitimately differ, so gating the
// build on an exact match would always fail. The deps are still computed + recorded.
...
export async function getDependencies(packageType: 'deb' | 'rpm', buildDir: string, applicationName: string, arch: string): Promise<string[]> {
	...
	// Add the tunnel binary, when present. Clawdius does not build the Rust
	// dev-tunnels CLI, so bin/<tunnelApplicationName> is absent and is skipped.
