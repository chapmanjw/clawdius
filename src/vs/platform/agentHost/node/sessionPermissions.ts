...
// assertPathIsSafe + resolveRealPathForNonexistent now live in the shared writePathCanonicalization module (so the
// trust gate and this manager use ONE canonicalizer) - imported at the top of this file.
...
export class SessionPermissionManager extends Disposable {
	...
	/** Override for {@link resolveRealPathForNonexistent}'s `realpath` call; set by tests only. */
	private readonly _realpath: ((fsPath: string) => Promise<string>) | undefined;
	...
	private async _resolveResourcesForApproval(resource: URI): Promise<URI[] | undefined> {
		...
		try {
			const resolved = URI.file(this._realpath ? await resolveRealPathForNonexistent(resource.fsPath, this._realpath) : await resolveRealPathForNonexistent(resource.fsPath));
