...
import { type MessageResourceAttachment, type ProtectedResourceMetadata } from '../../common/state/protocol/state.js';
...
suite('AgentService (node dispatcher)', () => {
	...
	setup(async () => {
		...
		stubAgent = new MockAgent('copilot');
		disposables.add(toDisposable(() => stubAgent.dispose()));
	});
	...
	suite('registerProvider', () => {
		...
		test('registers a provider successfully', () => {
			service.registerProvider(stubAgent);
			// No throw - success
		});
		...
		test('aggregates and deduplicates network diagnostics endpoints', async () => {
			const providerA: IAgent = stubAgent;
		...
		test('aggregates managed-settings diagnostics from capable providers', async () => {
			const provider: IAgent = stubAgent;
	...
	suite('disposeSession', () => {
		...
		test('dispatches to the correct provider and cleans up tracking', async () => {
			...
			assert.strictEqual(stubAgent.disposeSessionCalls.length, 1);
		});
		...
		test('deletes session data before removing the worktree', async () => {
			...
			svc.registerProvider(stubAgent);
			const session = await svc.createSession({ provider: 'copilot' });
	...
	suite('aggregation', () => {
		...
		test('listSessions uses SDK title when no custom title exists', async () => {
			service.registerProvider(stubAgent);
			stubAgent.sessionMetadataOverrides = { summary: 'Auto-generated Title' };
	...
	suite('authenticate', () => {
		...
		// Neutral protected-resource fixture for the generic token-store
		// tests below. Uses the same resource the stub agent advertises so
		// `authenticate` matches it and the token is stored.
		const TEST_PROTECTED_RESOURCE: ProtectedResourceMetadata = {
		...
		test('routes token to provider matching the resource', async () => {
			...
			assert.deepStrictEqual(stubAgent.authenticateCalls, [{ resource: 'https://api.github.com', token: 'ghp_test123' }]);
		});
		...
		test('returns not authenticated for unknown resource', async () => {
			...
			assert.deepStrictEqual({ result, token: service.getAuthToken({ resource: 'https://unknown.example.com' }), authenticateCalls: stubAgent.authenticateCalls }, {
		...
		test('stores an auth token for operation handlers', async () => {
			service.registerProvider(stubAgent);

			const result = await service.authenticate({ resource: TEST_PROTECTED_RESOURCE.resource, token: 'copilot-token' });

			assert.deepStrictEqual({ result, token: service.getAuthToken({ resource: TEST_PROTECTED_RESOURCE.resource, scopes: TEST_PROTECTED_RESOURCE.scopes_supported }), authenticateCalls: stubAgent.authenticateCalls }, {
				result: { authenticated: true },
				token: 'copilot-token',
				authenticateCalls: [{ resource: TEST_PROTECTED_RESOURCE.resource, token: 'copilot-token' }],
			});
		});
		...
		test('stores tokens for the same resource by scopes', async () => {
			...
			await service.authenticate({ resource: TEST_PROTECTED_RESOURCE.resource, scopes: ['read:user'], token: 'read-token' });
			await service.authenticate({ resource: TEST_PROTECTED_RESOURCE.resource, scopes: ['read:user', 'user:email'], token: 'profile-token' });

			assert.deepStrictEqual({
				readToken: service.getAuthToken({ resource: TEST_PROTECTED_RESOURCE.resource, scopes: ['read:user'] }),
				profileToken: service.getAuthToken({ resource: TEST_PROTECTED_RESOURCE.resource, scopes: ['user:email', 'read:user'] }),
				supersetToken: service.getAuthToken({ resource: TEST_PROTECTED_RESOURCE.resource, scopes: ['user:email'] }),
			}, {
		...
		test('fans out to every provider that owns the resource', async () => {
			...
			assert.deepStrictEqual({
				result,
				copilotCalls: stubAgent.authenticateCalls,
	...
	suite('shutdown', () => {
		...
		test('shuts down all providers', async () => {
			...
			stubAgent.shutdown = async () => { copilotShutdown = true; };
	...
	suite('restoreSession', () => {
		...
		test('restores the AH-owned workspaceless marker onto the summary _meta for any agent', async () => {
			...
			localService.registerProvider(stubAgent);
			await stubAgent.createSession();
			const sessionResource = (await stubAgent.listSessions())[0].session;
		...
		test('restores a session with message history', async () => {
			service.registerProvider(stubAgent);
			const { session } = await stubAgent.createSession();
		...
		test('restores chat drafts from session metadata', async () => {
			...
			(stubAgent as MockAgent & { getChatDraft(chat: URI): Promise<typeof draft | undefined> }).getChatDraft = chat => db.getChatDraft(chat) as Promise<typeof draft | undefined>;
			stubAgent.sessionMessages = [];
		...
		test('falls back to listing sessions when direct metadata restore fails', async () => {
			...
			stubAgent.getSessionMetadata = async () => {
				throw new Error('direct metadata unavailable');
			};
			const originalListSessions = stubAgent.listSessions.bind(stubAgent);
			let listSessionsCalled = false;
			stubAgent.listSessions = async () => {
				listSessionsCalled = true;
		...
		test('hydrates session customizations when restoring an existing session', async () => {
			...
			stubAgent.getSessionCustomizations = async () => {
	...
	suite('subagent membership sequencing', () => {
		...
		test('an inner tool call arriving before subagent_started is buffered and drained onto the subagent chat', async () => {
			...
			stubAgent.fireProgress({ kind: 'action', resource: URI.parse(parentChat), action: { type: ActionType.ChatToolCallStart, turnId: 'turn-1', toolCallId: 'tc-sub', toolName: 'task', displayName: 'Task', contributor: undefined, _meta: { toolKind: undefined, language: undefined } } });
			stubAgent.fireProgress({ kind: 'action', resource: URI.parse(parentChat), action: { type: ActionType.ChatToolCallReady, turnId: 'turn-1', toolCallId: 'tc-sub', invocationMessage: 'Delegating...', toolInput: undefined, confirmed: ToolCallConfirmationReason.NotNeeded } });

			// Inner tool arrives BEFORE subagent_started (buffered).
			stubAgent.fireProgress({ kind: 'action', resource: URI.parse(parentChat), parentToolCallId: 'tc-sub', action: { type: ActionType.ChatToolCallStart, turnId: 'turn-1', toolCallId: 'inner-1', toolName: 'read', displayName: 'Read', contributor: undefined, _meta: { toolKind: undefined, language: undefined } } });
			stubAgent.fireProgress({ kind: 'action', resource: URI.parse(parentChat), parentToolCallId: 'tc-sub', action: { type: ActionType.ChatToolCallReady, turnId: 'turn-1', toolCallId: 'inner-1', invocationMessage: 'Reading...', toolInput: undefined, confirmed: ToolCallConfirmationReason.NotNeeded } });

			// subagent_started arrives and drains the buffer.
			stubAgent.fireProgress({ kind: 'subagent_started', chat: URI.parse(parentChat), toolCallId: 'tc-sub', agentName: 'explore', agentDisplayName: 'Explore', agentDescription: 'Explores' });
		...
		test('a subagent chat survives subagent_completed (stays live and subscribable, its turn completed)', async () => {
			...
			stubAgent.fireProgress({ kind: 'subagent_completed', chat: URI.parse(parentChat), toolCallId: 'tc-sub' });
	...
	suite('subscriber refcount eviction', () => {
		...
		test('a restored idle session is evicted when its last subscriber drops', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual(
					stubAgent.releaseSessionCalls.map(u => u.toString()),
					[sessionResource.toString()],
					'provider releaseSession should be invoked for the evicted root',
				);
				assert.strictEqual(stubAgent.disposeSessionCalls.length, 0, 'eviction must not destructively dispose the session');
			});
		});
		...
		test('re-subscribing within the grace cancels the release', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(stubAgent.releaseSessionCalls.length, 0, 'releaseSession must not fire when the grace was cancelled');
			});
		});
	...
	suite('addSubscriber triggers uncommitted refresh', () => {
		...
		test('addSubscriber for <session>/changeset/uncommitted triggers the first git diff refresh', async () => {
			...
			stubAgent.resolvedWorkingDirectory = workingDirectory;
			stubAgent.sessionMetadataOverrides = { workingDirectories: workingDirectory ? [workingDirectory] : undefined };
	...
	suite('empty-session GC', () => {
		...
		test('an empty unsubscribed session is disposed after the grace period', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(stubAgent.disposeSessionCalls.length, 0, 'no GC before grace expires');

				// After the grace period, the session is disposed entirely.
				await new Promise(resolve => setTimeout(resolve, 30_000));
				assert.deepStrictEqual(
					stubAgent.disposeSessionCalls.map(u => u.toString()),
					[sessionResource.toString()],
		...
		test('a session with at least one turn is not GC-disposed', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(stubAgent.disposeSessionCalls.length, 0, 'session with turns must not be GC-disposed');
			});
		});
		...
		test('resubscribe within the grace period cancels GC', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(stubAgent.disposeSessionCalls.length, 0, 'GC must be cancelled after resubscribe');
			});
		});
		...
		test('GC is rearmed after a resubscribe-then-unsubscribe cycle', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(stubAgent.disposeSessionCalls.length, 0, 'rearmed timer not yet fired');
				await new Promise(resolve => setTimeout(resolve, 2_000));
				assert.strictEqual(stubAgent.disposeSessionCalls.length, 1, 'rearmed timer fires after fresh 30s');
			});
		});
		...
		test('createSession on the same URI cancels a pending GC', () => {
			...
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				await new Promise(resolve => setTimeout(resolve, 30_000));
				assert.strictEqual(stubAgent.disposeSessionCalls.length, 0, 'createSession on same URI must cancel pending GC');
			});
		});
		...
		test('a restored session that loaded zero turns is never GC-disposed', () => {
			...
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual({
					disposed: stubAgent.disposeSessionCalls.map(u => u.toString()),
					released: stubAgent.releaseSessionCalls.map(u => u.toString()),
				}, {
		...
		test('a session restored during the grace window is not GC-disposed', () => {
			...
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(stubAgent.disposeSessionCalls.length, 0, 'a session restored mid-grace must not be GC-disposed');
			});
		});
		...
		test('a session truncated back to zero turns is not GC-disposed', () => {
			...
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(stubAgent.disposeSessionCalls.length, 0, 'a used session must not be GC-disposed after truncation');
			});
		});
	});
	...
	suite('worktree working directory', () => {
		...
		test('createSession uses agent-resolved working directory in state', async () => {
			...
			stubAgent.resolvedWorkingDirectory = worktreeDir;
			service.registerProvider(stubAgent);
		...
		test('createSession falls back to config working directory when agent does not resolve', async () => {
			...
			stubAgent.resolvedWorkingDirectory = undefined;
			service.registerProvider(stubAgent);
		...
		test('restoreSession uses agent working directory in state', async () => {
			...
			const worktreeDir = URI.file('/source/repo.worktrees/agents-xyz');
			stubAgent.sessionMetadataOverrides = { workingDirectories: worktreeDir ? [worktreeDir] : undefined };
			service.registerProvider(stubAgent);
