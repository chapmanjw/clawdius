				...
				return this._taskSystem!.terminateAll().then((responses) => {
					...
					if (success) {
						...
						return this._dialogService.confirm({
							// CLAWDIUS-BEGIN brand sweep (shutdown confirm dialog self-reference)
							message: nls.localize('TaskSystem.noProcess', 'The launched task doesn\'t exist anymore. If the task spawned background processes exiting Clawdius might result in orphaned processes. To avoid this start the last background process with a wait flag.'),
							// CLAWDIUS-END
