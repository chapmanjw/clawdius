...
import { IAgentChatDataChange, IAgentMaterializeSessionEvent, IAgentSpawnChatEvent, AgentSession, AgentSignal, GITHUB_REPO_PROTECTED_RESOURCE } from '../../common/agentService.js';
import { AgentHostClaudeMultiRootEnabledConfigKey } from '../../common/agentHostSchema.js';
import { AgentFeedbackAttachmentDisplayKind } from '../../common/meta/agentFeedbackAttachments.js';
import { ActionType } from '../../common/state/sessionActions.js';
import { CustomizationLoadStatus, CustomizationType, MessageAttachmentKind, MessageKind, ResponsePartKind, TurnState, type Turn, ChatInputAnswerState, ChatInputAnswerValueKind, ChatInputResponseKind, SessionInputAnswerState, SessionInputAnswerValueKind, SessionInputResponseKind, SessionStatus, ToolCallStatus, ToolResultContentType, buildChatUri, buildDefaultChatUri, buildSubagentChatUri, buildSubagentSessionUri, customizationId, parseChatUri, parseDefaultChatUri, type ChatInputRequest, type ClientPluginCustomization, type Customization, type PluginCustomization, type SessionConfigState, type SessionInputRequest, type ToolDefinition } from '../../common/state/sessionState.js';
...
/**
 * A two-turn source transcript (`u1`/`a1`, `u2`/`a2`) used by the fork tests.
 * Forking at `u1` keeps `[u1]` inclusive, anchored on that turn's last
 * assistant envelope `a1`.
 */
...
/**
 * The {@link IFileService} + {@link INativeEnvironmentService} pair the merged
 * ClaudeAgentSession needs at construction (the customization disk scan /
 * watcher reads `userHome`). Nothing is seeded under `userHome`, so the scan is
 * deterministically empty — these only exist so `new ClaudeAgentSession` can
 * read `userHome` and start its watcher without throwing.
 */
...
class FakeClaudeAgentSdkService implements IClaudeAgentSdkService {
	...
	/**
	 * Programmable results for the FakeQuery's `supportedCommands` /
	 * `supportedAgents` / `mcpServerStatus` control calls. `undefined` means
	 * "not modeled" (the FakeQuery throws) for the agents/mcp fields;
	 * commands defaults to empty since most suites don't exercise it.
	 */
	...
	/**
	 * Optional gate blocking the query's async-iterator `return()` (its
	 * teardown / subprocess-exit path). Tests use it to hold a live query
	 * "still exiting" while asserting that dependent cleanup (e.g.
	 * `deleteSession`) hasn't run yet. Resolves immediately when undefined.
	 */
	...
	async query(_params: { prompt: string | AsyncIterable<SDKUserMessage>; options?: Options }): Promise<Query> {
		throw new Error('FakeClaudeAgentSdkService: query not modeled');
	}

	/**
	 * Records every {@link forkSession} call so tests can assert the resolved
	 * fork anchor. Tests capture the forwarded `(sessionId, options)` and
	 * program the resulting new session id (or a rejection); defaults to a
	 * deterministic `forked-<source>` id so suites that don't care about the
	 * exact value don't have to set it.
	 */
	readonly forkSessionCalls: { sessionId: string; options: ForkSessionOptions | undefined }[] = [];
	...
	/**
	 * Programmable session deletion (remove-all truncation). Tests capture
	 * the deleted ids; `deleteSessionRejection` simulates an SDK throw.
	 */
...
function createTestContext(
	...
	overrides?: { logService?: ILogService; database?: TestSessionDatabase; productService?: IProductService; userHome?: URI; rootConfig?: Record<string, unknown> },
): ITestContext {
	...
	return { agent, sdk, sessionData, stateManager, configService, instantiationService, fileService };
}
...
suite('ClaudeAgent', () => {
	...
	// CLAWDIUS: the rest of upstream's block here exercises the CAPI proxy / Copilot model catalog, which the
	// fork removes. This one test covers the multi-root capability gate, which the fork does adopt.
	...
	test('phase-stub graduation: abortSession + changeModel no longer throw', async () => {
		// Both methods are idempotent on unknown default chat URIs (no-op rather than throw)
	...
	test('resume keeps the existing overlay model (materialize does not clobber on isResume)', async () => {
		...
		// resume-preserves-overlay-model assertion is obsolete; assert the resume returned metadata.
		const metadataAfterResume = await agent.getSessionMetadata(created.session);
		assert.ok(metadataAfterResume, 'resume must return session metadata');
	});
	...
	test('createSession({ fork }) forks at the anchor uuid, then materializes lazily on first sendMessage', async () => {
		...
		// sendMessage resumes from disk (Options.resume).
		const { agent, sdk } = createTestContext(disposables);
	...
	test('createSession({ fork }) ignores requested working directories and inherits the live source set', async () => {
		...
		await agent.authenticate(GITHUB_REPO_PROTECTED_RESOURCE.resource, 'tok');
	...
	test('createSession({ fork }) with a create-config model override persists it on the fork', async () => {
		...
		// CLAWDIUS: model ids flow raw into the SDK Options.model (no CAPI-style
		// dot→hyphen translation; that seam belonged to the removed Copilot/CAPI
		// transport).
	...
	test('cold resume recovers the additional directories from the persisted overlay', async () => {
		...
		await ctxA.agent.authenticate(GITHUB_REPO_PROTECTED_RESOURCE.resource, 'tok');
		...
		await ctxB.agent.authenticate(GITHUB_REPO_PROTECTED_RESOURCE.resource, 'tok');
	...
	test('text content_block emits SessionResponsePart(Markdown) before SessionDelta', async () => {
		...
		assert.deepStrictEqual({
			...
			// 1.127: action signals target the chat CHANNEL uri (ahp-chat://…),
			// not the raw session uri — derive it from the same helper the code uses.
			session: URI.parse(buildDefaultChatUri(created.session)).toString(),
		});
	});
	...
	test('subagent spawn mirrors onto onDidSpawnChat while the subagent signals still flow', async () => {
	...
	test('mapper throwing on a malformed stream_event is logged and the turn continues', async () => {
		...
		disposables.add(agent.onDidSessionProgress(s => {
			// 1.127: signals carry the chat CHANNEL uri (ahp-chat://…) rather than
			// the raw session uri, so recover the owning session before matching.
			const channel = s.kind === 'action' ? s.resource : s.chat;
			if (parseDefaultChatUri(channel) === sessionUri.toString()) {
				observed.push(s);
			}
		}));
	...
	test('createSession.model round-trips through the per-session DB to listSessions[].model', async () => {
		...
		sdk.nextQueryMessages = [makeSystemInitMessage(sessionId), makeResultSuccess(sessionId)];
		await agent.chats.sendMessage(URI.parse(buildDefaultChatUri(created.session)), 'hi', undefined, undefined, 'turn-1');
		...
		// NOTE: `model` was removed from IAgentSessionMetadata in 1.127, so the
		// original model round-trip assertion is obsolete. What survives is the
		// SDK summary surfacing through listSessions (there is no replacement
		// metadata field for `model`).
		assert.deepStrictEqual({
	...
	test('onClientToolCallComplete is a benign no-op for an unknown toolCallId', () => {
		...
		assert.doesNotThrow(() => {
			agent.onClientToolCallComplete(session, URI.parse(buildDefaultChatUri(session)), 'toolu_unknown', { success: true, pastTenseMessage: 'ran' });
		});
	});
	...
	test('onClientToolCallComplete walks subagent URIs to the root session', () => {
		...
		assert.doesNotThrow(() => {
			agent.onClientToolCallComplete(depth2, URI.parse(buildDefaultChatUri(depth2)), 'tu_anything', { success: true, pastTenseMessage: 'ran' });
		});
	});
...
suite('ClaudeAgent (_handleCanUseTool)', () => {
	...
	test('Test 7 — pending_confirmation signal carries the correct shape', async () => {
		...
		assert.deepStrictEqual(captured, {
			...
			// 1.127: the signal targets the chat CHANNEL uri, not the session uri.
...
suite('ClaudeAgent (MCP elicitation translation)', () => {
	...
	async function materialize(): Promise<{
		...
		const ctx = createTestContext(disposables);
		await ctx.agent.authenticate(GITHUB_REPO_PROTECTED_RESOURCE.resource, 'tok');
		const created = await ctx.agent.createSession({ workingDirectories: [URI.file('/work')] });
		const sessionId = AgentSession.id(created.session);
	...
	test('form-mode elicitation surfaces ChatInputRequested and returns accepted content', async () => {
		...
		const promise = onElicitation(
			...
			{ signal: new AbortController().signal, requestId: 'el_form_accept' },
		);
		await tick();
	...
	test('declined form-mode elicitation returns a decline result', async () => {
		...
		const promise = onElicitation(
			...
			{ signal: new AbortController().signal, requestId: 'el_form_decline' },
		);
		await tick();
	...
	test('aborting the SDK signal cancels a parked elicitation', async () => {
		...
		const promise = onElicitation(
			{ serverName: 'm', message: 'q', mode: 'form', requestedSchema: { type: 'object', properties: { side: { type: 'string' } } } },
			{ signal: controller.signal, requestId: 'el_abort_parked' },
		);
		await tick();
	...
	test('url-mode elicitation surfaces the url with no questions and accepts with no content', async () => {
		...
		const promise = onElicitation(
			...
			{ signal: new AbortController().signal, requestId: 'el_url_accept' },
		);
		await tick();
	...
	test('a pre-aborted signal cancels without ever parking', async () => {
		...
		const result = await onElicitation(
			{ serverName: 'm', message: 'q', mode: 'form', requestedSchema: { type: 'object', properties: { side: { type: 'string' } } } },
			{ signal: controller.signal, requestId: 'el_pre_aborted' },
		);
	...
	test('a url-mode request with no url cancels without surfacing a prompt', async () => {
		...
		const result = await onElicitation(
			...
			{ signal: new AbortController().signal, requestId: 'el_url_no_url' },
		);
	...
	test('a form with no representable fields cancels without surfacing a prompt', async () => {
		...
		const result = await onElicitation(
			...
			{ signal: new AbortController().signal, requestId: 'el_form_no_fields' },
		);
...
suite('ClaudeAgent — customizations', () => {
	...
	function buildCtxWith(pluginManager: FakeAgentPluginManager): ITestContext {
		...
		const fileEnvServices = claudeFileEnvServices(disposables);
		const fileService = fileEnvServices.find((entry): entry is [typeof IFileService, IFileService] => entry[0] === IFileService)![1];
		const services = new ServiceCollection(
			...fileEnvServices,
			[ILogService, logService],
			[ISessionDataService, sessionData],
	...
	test('workspace-trust revocation rebuilds the session to drop trusted engine state', async () => {
		...
		const sessionId = AgentSession.id(created.session);
		sdk.nextQueryMessages = [makeSystemInitMessage(sessionId), makeResultSuccess(sessionId)];
		await agent.chats.sendMessage(URI.parse(buildDefaultChatUri(created.session)), 'first', undefined, undefined, 'turn-1');
	...
	test('a late workspace-trust grant rebuilds the session on the next send to restore trusted engine state', async () => {
		...
		configService.updateRootConfig({ [AgentHostTrustConfigKey.Trust]: { [AgentHostTrustKey.Trusted]: true } });
		await agent.chats.sendMessage(URI.parse(buildDefaultChatUri(created.session)), 'second', undefined, undefined, 'turn-2');
...
suite('ClaudeAgent - Clawdius native mode (no Copilot)', () => {
	...
	test('createSession runs without an auth gate (no AHP_AUTH_REQUIRED throw)', async () => {
		const { agent } = createTestContext(disposables, { productService: ClawdiusProductService });
		const result = await agent.createSession({ workingDirectories: [URI.file('/workspace')] });
