	...
	const task = () => {
		...
		const deps = es.merge(cleanedDeps, osProxyResolverPlatformPackage)
			...
			.pipe(createAsar(path.join(process.cwd(), 'node_modules'), [
				...
				// @microsoft/mxc-sdk ships the wxc-exec.exe helper used by the Windows
				// terminal sandbox engine (see src/vs/platform/sandbox). It is spawned
				// as a subprocess and is self-locating, so it must be a real file on
				// disk, not packed inside the archive.
	...
	return async () => {
		...
		const patchPromises = deps.map<Promise<unknown>>(async dep => {
			...
			await rcedit(fullPath, {
				...
				'version-string': {
					'CompanyName': 'John Chapman',
					'FileDescription': product.nameLong,
					'FileVersion': packageJson.version,
					'InternalName': basename,
					'LegalCopyright': 'Copyright (C) 2026 John Chapman; portions Copyright (C) Microsoft Corporation',
