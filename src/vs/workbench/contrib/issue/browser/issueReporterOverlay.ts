...
export class IssueReporterOverlay {
	...
	private createStep0Attachments(): void {
		...
		subtitle.textContent = localize('screenshotsSubtitle', "You can add up to {0} screenshots or videos. Navigate Clawdius and choose when to capture.", MAX_ATTACHMENTS);
	...
	private getAllSourceOptions(): { label: string; value: IssueSource }[] {
		return [
			// CLAWDIUS-BEGIN brand sweep (issue reporter source labels)
			{ label: product.nameLong || localize('vscode', "Clawdius"), value: IssueSource.VSCode },
			{ label: localize('agentsWindow', "Agents Window"), value: IssueSource.AgentsWindow },
			{ label: localize('extensionSource', "A Clawdius extension"), value: IssueSource.Extension },
			// CLAWDIUS-END
	...
	private getIssueSourceLabel(): string {
		switch (this.selectedIssueSource) {
			case IssueSource.VSCode:
				return product.nameLong || localize('vscode', "Clawdius");
			case IssueSource.AgentsWindow:
				return localize('agentsWindow', "Agents Window");
			case IssueSource.Extension:
				return this.selectedExtension?.displayName || this.selectedExtension?.name || localize('extensionSource', "A Clawdius extension");
			case IssueSource.Marketplace:
			...
			this.createDiagSection(diagContainer, {
				...
				renderContent: (container) => {
					...
					if (modelData.versionInfo) {
						this.addDiagRow(sysTable, 'Clawdius', modelData.versionInfo.vscodeVersion);
	...
	private generateIssueDetailsMd(): string {
		...
		const rows: [string, string | undefined][] = [
			...
			['Clawdius Version', modelData.versionInfo?.vscodeVersion ?? product.version],
	...
	private generateSystemInfoMd(): string {
		...
		if (modelData.versionInfo) {
			rows.push(
				['Clawdius Version', modelData.versionInfo.vscodeVersion],
