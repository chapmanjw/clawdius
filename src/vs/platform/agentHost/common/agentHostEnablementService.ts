...
// CLAWDIUS-BEGIN agent host on for clawdius (see the `chat.agentHost.enabled` default below)
...
// CLAWDIUS-BEGIN load fork-owned agent-host settings (native Claude default provider, workspace-trust gate, CLI engine)
import './clawdiusAgentHost.config.contribution.js';
// CLAWDIUS-END
...
configurationRegistry.registerConfiguration({
	...
	properties: {
		[agentHostEnabledSettingId]: {
			...
			// Clawdius (entitlementUrl empty) powers the Agents window via the agent-host Claude provider, so the host
			// must spawn in dev AND built/stable. This key is read in the main process where product.json
		...
		'chat.editor.localAgent.enabled': {
			...
			description: nls.localize('chat.editor.localAgent.enabled', "When enabled, shows the Clawdius local chat harness in the chat picker. This setting is ignored in virtual workspaces, where the local chat harness is always available."),
			// CLAWDIUS-END
			default: true,
			tags: ['experimental'],
			experiment: { mode: 'startup' },
