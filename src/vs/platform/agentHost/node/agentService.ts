...
import { AgentProvider, AgentSession, IAgent, IAgentCreateChatOptions, IAgentCreateSessionConfig, IAgentHostAuthTokenRequest, IAgentMaterializeSessionEvent, IAgentResolveSessionConfigParams, IAgentService, IAgentSessionConfigCompletionsParams, IAgentSessionMetadata, AuthenticateParams, AuthenticateResult, IMcpNotification, IRestoredSubagentSession } from '../common/agentService.js';
...
export class AgentService extends Disposable implements IAgentService {
	...
	constructor(
		...
		// CLAWDIUS: the commit + pull-request changeset operations are removed entirely. Commit's handler
		// depended on the deleted copilotApiService; the pull-request op (createPullRequest / enable-auto-merge
		// -> api.github.com) is a GitHub write surface with no place in a no-account, zero-egress fork, so its
		// provider + handler are deleted. Only the local-only sync + discard-changes operations remain.
		...
		this._sideEffects = this._register(instantiationService.createInstance(AgentSideEffects, this._stateManager, {
			...
			onTurnComplete: session => {
				// Refresh git state for the session. Upstream also fired a GitHub PR-lookup here; dropped -
				// no IDE account, zero egress (and the getProtectedResources()==[] gate would no-op it anyway).
	...
	private async _doRestoreSession(session: URI, sessionStr: string): Promise<void> {
		...
		// CLAWDIUS: upstream also called _gitStateService.attachSessionGitHubPullRequest(sessionStr) here -
		// dropped (GitHub PR lookup, api.github.com; no IDE account, zero egress).
	}
