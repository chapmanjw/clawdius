...
// CLAWDIUS-BEGIN native main-IDE clawdius container (Phase 2b: read-only workflow store)
// The durable, READ-ONLY index of Claude Code "Ultracode" workflow runs, for the native main-IDE Workflows
// view. Reads the CLI's run records under ~/.claude/projects/<encoded-cwd>/<sessionId>/workflows/wf_*.json
// (completed) and subagents/workflows/<runId> journals (live), and publishes a run list. It depends ONLY on
// file/path/log services - NO agent-host control. Per-workflow Managed cancel/steer (which needs the agent
// host) lives separately and lands with the native chat (Phase 3) behind a control provider; this store is
// observe-only by design, so the main IDE never carries hidden live controls.
