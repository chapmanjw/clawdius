...
suite('LocalChatSessionsProvider', () => {
	...
	test('keeps empty default-vendor resolution pending until live default-vendor models arrive', () => {
		const store = leaks.add(new DisposableStore());
		const { instantiationService } = createFixture(store);
		...
		const provider = store.add(instantiationService.createInstance(LocalChatSessionsProvider));
		const emptyDefault = provider.getModelsSnapshot('session', `${COPILOT_VENDOR_ID}/remembered`);
		...
		const liveDefault = provider.getModelsSnapshot('session', `${COPILOT_VENDOR_ID}/remembered`);

		assert.deepStrictEqual({
			emptyDefault: emptyDefault.desiredModelResolution,
			emptyByok: emptyByok.desiredModelResolution,
			liveDefault: liveDefault.desiredModelResolution,
			models: liveDefault.models.map(model => model.identifier),
			modelTarget: liveDefault.modelTarget,
		}, {
			...
			models: [`${COPILOT_VENDOR_ID}/other`],
