...
export class ClaudeAgentSession extends Disposable {
	...
	/**
	 * Transport the session materialized under. Clawdius is always native
	 * (no CAPI proxy), so this is fixed to `native`; it gates
	 * {@link _enrichSignalWithCredits} so native turns never carry a Copilot
	 * credits overlay (the proxy is the only credit source).
	 */
	private _transportKind: 'proxy' | 'native' = 'native';
	...
	private _watchCustomizations(directories: readonly URI[] | undefined): void {
		...
		// sessions window forwards trust at the SESSION layer, the main window at the ROOT layer. Scoped to the
		// per-call watcher store (not `this._register`) so re-invoking `_watchCustomizations` doesn't stack
		// duplicate listeners.
		store.add(this._configurationService.onDidRootConfigChange(() => this._onTrustConfigMaybeChanged()));
		store.add(this._configurationService.onDidSessionConfigChange(changed => {
			if (changed.session === this.sessionUri.toString()) {
		...
		if (!ctx.isResume) {
			try {
				await this._metadataStore.write(this._storageUri, {
					...
					model: this._provisionalModel,
					permissionMode,
					// CLAWDIUS: the CAPI proxy transport is removed, so the fork is native-only and there is
					// no `ctx.transport` to read a kind from.
					transport: 'native',
	...
	async send(prompt: SDKUserMessage, turnId: string): Promise<void> {
		...
		if (trustDiverged || this.toolDiff.hasDifference || this.clientCustomizationsDiff.hasDifferenceFrom(this._desiredClientPluginPaths()) || this._pendingResumeSessionAt !== undefined) {
