	...
	private filterServers(): void {
		...
		if (extensionServers.length > 0) {
			...
			entries.push({
				...
				isFirst,
				description: localize('extensionGroupDescription', "MCP servers contributed by installed Clawdius extensions."),
				collapsed,
			});
			if (!collapsed) {
		...
		if (otherBuiltinServers.length > 0) {
			...
			entries.push({
				...
				isFirst,
				description: localize('builtInGroupDescription', "MCP servers built into Clawdius. These are available automatically."),
				collapsed,
			});
			if (!collapsed) {
