...
import { COPILOT_VENDOR_ID, ILanguageModelChatMetadata, ILanguageModelsService } from '../../../../../../workbench/contrib/chat/common/languageModels.js';
...
suite('CopilotChatSessionsProvider', () => {
	...
	// CLAWDIUS: `_isCopilotCliAvailable` ("no copilot cli session type") and the
	// Cloud push in `sessionTypes` ("no copilot cloud session type") both gate
	// on `product.defaultChatAgent?.entitlementUrl`, which is empty in
	// Clawdius (no Copilot account) — so CopilotCLISessionType and
	// CopilotCloudSessionType can never appear, in tests or at runtime. Claude
	// is the only session type this provider can ever surface, so the counts
	// below are 0 or 1, not the upstream 2/3.
	...
	test('sessionTypes excludes Claude when setting is disabled', () => {
		...
		assert.strictEqual(provider.sessionTypes.length, 0);
	...
	// CLAWDIUS: CopilotCLISessionType can never appear (see the "no copilot cli
	// session type" gate on `_isCopilotCliAvailable` — there is no
	// extension-host Copilot CLI harness in Clawdius). The hideExtensionHost /
	// chat.agentHost.enabled interplay below it is therefore dead code for
	// this provider; these three tests now assert the entry stays absent
	// either way, while still exercising the config-change wiring so a
	// regression there (e.g. dropping the `affectsConfiguration` check) is
	// still caught.
	...
	// CLAWDIUS: `COPILOT_VENDOR_ID` (languageModels.ts) is repurposed to the
	// fork's own built-in vendor id `'clawdius'` — Copilot is eliminated, so
	// the "stay pending until live models arrive" grace period now belongs to
	// the Claude/Clawdius default vendor, not a literal `'copilot'` vendor
	// string (which resolves like any ordinary third-party vendor once
	// `hasResolvedVendor` is true).
	test('Copilot CLI keeps an empty default-vendor catalog pending until live models arrive', () => {
		...
		const empty = provider.getModelsSnapshot(session.sessionId, `${COPILOT_VENDOR_ID}/remembered`);

		models.set(`${COPILOT_VENDOR_ID}/other`, {
		...
		const live = provider.getModelsSnapshot(session.sessionId, `${COPILOT_VENDOR_ID}/remembered`);
		...
		assert.deepStrictEqual({
			...
			empty: { resolution: { kind: 'pending', identifier: `${COPILOT_VENDOR_ID}/remembered` }, modelTarget: CopilotCLISessionType.id },
			live: { resolution: { kind: 'unavailable', identifier: `${COPILOT_VENDOR_ID}/remembered` }, modelTarget: CopilotCLISessionType.id },
		});
	});
