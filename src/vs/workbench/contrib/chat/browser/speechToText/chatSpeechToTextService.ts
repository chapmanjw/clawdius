...
export class ChatSpeechToTextService extends Disposable implements IChatSpeechToTextService {
	...
	private _getBackend(): DictationBackend {
		// CLAWDIUS-BEGIN on-device dictation only
		// The `mai` backend streams microphone audio to a cloud voice service. Clawdius drops it from the
		// `dictation.model` enum, but enum values are advisory - settings.json can still name it. Pin the
		// backend here so no configuration path can turn dictation into an off-box audio stream.
