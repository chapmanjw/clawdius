	...
	constructor(
		...
		this._sideEffects = this._register(instantiationService.createInstance(AgentSideEffects, this._stateManager, {
			...
			onUserMessage: (session, text) => {
				// Record the GitHub issues the message references on the session. Local-only: this parses the
				// message text for owner/repo#123 references and stores the derived URLs on session state.
