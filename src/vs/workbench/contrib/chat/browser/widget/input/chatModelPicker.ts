...
export class ModelPickerWidget extends Disposable {
	...
	private _renderLabel(): void {
		...
		// Clawdius (empty entitlementUrl) is always PRU; surface the clickable effort + context-size pills
		// (the visible "Ultracode effort" selector) rather than folding config into a label suffix. The click
		// handlers are wired unconditionally. Consumed by the effortConfig/tokensConfig gate below.
		const showConfigPills = !!this._entitlementService.quotas.usageBasedBilling || !this._productService.defaultChatAgent?.entitlementUrl;
		// CLAWDIUS-END
