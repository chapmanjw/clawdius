...
class FakeClaudeAgentSdkService implements IClaudeAgentSdkService {
	...
	/** Records every {@link forkSession} call so tests can assert the resolved fork anchor. */
	readonly forkSessionCalls: Array<{ sessionId: string; options: unknown }> = [];

	async forkSession(sessionId: string, options?: unknown): Promise<{ sessionId: string }> {
		this.forkSessionCalls.push({ sessionId, options });
		return { sessionId: `forked-${sessionId}` };
	}

	async deleteSession(): Promise<void> {
		/* no-op: no fork/truncate test in this suite drives sdk.deleteSession */
	}
...
suite('ClaudeAgent', () => {
	...
	test('createSession({ fork }) forks at the resolved anchor into a new non-provisional session', async () => {
		// Upstream 1.127 implemented fork (`_forkSession`): it resolves the
		// protocol `turnId` to its SDK envelope uuid via `resolveForkAnchorUuid`,
		...
		sdk.sessionMessagesById.set('src-uuid', [
			{
				type: 'user',
				uuid: 'turn-1',
				session_id: 'src-uuid',
		...
		// The forked session's working directory is recovered from the SDK's
		// session info for the newly minted (`forked-…`) id.
		sdk.sessionList = [{ sessionId: 'forked-src-uuid', cwd: '/work', summary: '', lastModified: 1234 }];
		...
		assert.deepStrictEqual({
			...
			workingDirectory: result.workingDirectory?.toString(),
			provisional: result.provisional,
			forkCalls: sdk.forkSessionCalls,
			startupCallCount: sdk.startupCallCount,
			listSessionsCallCount: sdk.listSessionsCallCount,
		}, {
			session: 'claude:/forked-src-uuid',
			workingDirectory: 'file:///work',
			provisional: undefined,
			forkCalls: [{ sessionId: 'src-uuid', options: { upToMessageId: 'a1' } }],
			startupCallCount: 0,
			listSessionsCallCount: 0,
		});
	});
...
suite('ClaudeAgent — Phase 11 customizations', () => {
	...
	test('getSessionCustomizations resolves against a provisional session', async () => {
		...
		// 1.127: getSessionCustomizations now folds in the discovered/built-in
		// tier (curated agents + skills pre-materialize) alongside the
		// client-pushed refs, so assert the client-pushed entry still resolves
		// rather than pinning the total projection size.
		assert.ok(customizations.some(c => c.uri === 'https://a'), 'client-pushed customization resolves on a provisional session');
	});
	...
	test('getSessionCustomizations swallows SDK snapshot failure and returns the client-pushed projection', async () => {
		...
		const customizations = await agent.getSessionCustomizations!(created.session);
		// 1.127: the projection also carries the discovered/built-in tier, so the
		// client-pushed slice is no longer the whole result — assert it survived
		// the SDK snapshot failure and still leads the projection (client-pushed
		// tier is emitted before the discovered tier).
		assert.ok(customizations.some(c => c.uri === 'https://a'), 'client-pushed projection survives SDK snapshot failure');
