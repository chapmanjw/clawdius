...
import { AgentHostClaudeAgentEnabledEnvVar, AgentHostClaudeSdkRootEnvVar, IAgentService, isAgentEnabled } from '../common/agentService.js';
...
function parseServerOptions(): IServerOptions {
	...
	// this flag or the matching env var, otherwise the provider will not register.
...
async function main(): Promise<void> {
	...
	// CLAWDIUS: upstream also passes a Copilot API service and the Codex provider configuration here;
	// both subsystems are stripped from the fork, so neither argument exists. The checkpoint service is
	// created inside AgentService and read back via `agentService.checkpointService` below.
	const agentService = new AgentService(logService, fileService, sessionDataService, productService, gitService, rootConfigResource, telemetryService, fileMonitorService, fetchFn);
