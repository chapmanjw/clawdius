...
const chatViewDescriptor: IViewDescriptor = {
	...
	// CLAWDIUS-BEGIN hide the old chat view entirely in Clawdius (kept REGISTERED, never visible)
	when: clawdiusOldChatIsDefault ? ContextKeyExpr.and(
...
export class ChatCompatibilityNotifier extends Disposable implements IWorkbenchContribution {
	...
	private registerWelcomeView(chatExtension: IExtension) {
		...
		const mainMessage = localize('chatFailErrorMessage', "Chat failed to load because the installed version of the Clawdius Chat extension is not compatible with this version of {0}. Please ensure that the Clawdius Chat extension is up to date.", this.productService.nameLong);
		...
		const versionMessage = `Clawdius Chat version: ${chatExtension.version}`;
