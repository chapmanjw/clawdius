...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		'dictation.enabled': {
			...
			// CLAWDIUS-BEGIN dictation is opt-in: first use downloads a model (~145-800MB) from a third-party host, so it must not be on by default
			default: false,
			// CLAWDIUS-END
			tags: ['experimental'],
			policy: {
		...
		'dictation.model': {
			type: 'string',
			enum: [
				// CLAWDIUS-BEGIN on-device transcription only
			...
			markdownEnumDescriptions: [
				// CLAWDIUS-BEGIN on-device transcription only (the cloud 'mai' model streams audio off-box)
				nls.localize('dictation.model.nemotronStreaming', "NVIDIA Nemotron 3.5 multilingual streaming RNN-T, run on-device. Works offline; no audio leaves the device. Automatic language selection follows the system or browser locale, with model detection as a fallback. Downloaded on first use and cached on disk."),
				// CLAWDIUS-END
			],
			markdownDescription: nls.localize('dictation.model', "The model used for dictation. The model downloads on first use and runs locally; no audio leaves the device."),
...
// CLAWDIUS: the Claude Code usage status entry + dashboard moved to contrib/clawdius/browser/usage (registered in clawdius.contribution.ts).
...
if (product.defaultChatAgent?.entitlementUrl) {
	...
	agentPluginDiscoveryRegistry.register(new SyncDescriptor(ClaudeCliAgentPluginDiscovery), AgentPluginDiscoveryPriority.CopilotCli);
}
// CLAWDIUS-END
