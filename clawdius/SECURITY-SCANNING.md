...
2. Forbidden content (`script/clawdius/scan-forbidden.js`): Copilot / GitHub-Copilot branding,
   known Microsoft telemetry keys, and Amazon-internal terms (from an env-referenced wordlist that
   is never committed via `CLAUDIUS_INTERNAL_WORDLIST`).
...
- The forbidden-content scan inspects only Clawdius-authored paths (`clawdius/**`,
  ...
  `CLAUDIUS-BEGIN` marker. It does not flag the upstream tree.
