...
function createTestContext(
	...
	overrides?: { logService?: ILogService; database?: TestSessionDatabase; productService?: IProductService; userHome?: URI },
): ITestContext {
...
suite('ClaudeAgent (INTERACTIVE_CLAUDE_TOOLS)', () => {
	...
	test('Test 12 — AskUserQuestion: surfaces SessionInputRequested, returns updatedInput keyed by question text', async () => {
		...
		}, { signal: new AbortController().signal, toolUseID: 'tu_ask', requestId: 'req_ask' });
		await tick();
	...
	test('Test 13 — AskUserQuestion: cancel returns deny with production wording', async () => {
		...
		}, { signal: new AbortController().signal, toolUseID: 'tu_ask_cancel', requestId: 'req_ask_cancel' });
		await tick();
